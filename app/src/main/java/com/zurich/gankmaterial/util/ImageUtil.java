...
public class ImageUtil {
    public static Uri saveImage(Context context, String url, Bitmap bitmap, ImageView imageView, String tag, boolean isShowTips) {
        //图片名称处理
        String[] strings = url.substring(url.lastIndexOf("/") + 1).split("\\.");
        String fileName = strings[0] + "." + strings[1];
        //创建文件路径
        File fileDir = new File(GankConstans.WELFARE_DIR);
        if (!fileDir.exists()) {
            fileDir.mkdir();
        }
        //创建文件
